...
def get_datasets_order_expansion_scenario(opt):
    loader_dict = dict()
    ...
    d_label_idxs = sample_nodes_within_range(tr_ages, opt.min_rank, opt.max_rank)
    ...
    loader_dict['train_lb'] = DataLoader(NodeLabeled(tr_imgs[d_label_idxs], tr_ages[d_label_idxs]),
                                         batch_size=opt.batch_size, shuffle=True, drop_last=True, num_workers=opt.num_workers)
    loader_dict['train_ulb'] = DataLoader(NodeUnlabeled(tr_imgs[d_unlabel_idxs], tr_ages[d_unlabel_idxs], len(d_label_idxs)*opt.ulb_ratio),
                                          batch_size=opt.batch_size*opt.ulb_ratio, shuffle=True, drop_last=True, num_workers=opt.num_workers)
    loader_dict['test'] = DataLoader(Reference([te_imgs, te_ages], [tr_imgs[d_label_idxs], tr_ages[d_label_idxs]]),
                                     batch_size=opt.batch_size, shuffle=False, drop_last=False, num_workers=opt.num_workers)

    loader_dict['vanilla_ulb'] = DataLoader(Vanilla(tr_imgs[d_unlabel_idxs], tr_ages[d_unlabel_idxs]),
...
class NodeUnlabeled(Dataset):
    ...
    def __getitem__(self, item):
        ...
        item = rng.choice(self.n_pairs, 1)[0]
        ...
        ref_age = self.labels[ref_idx]
        gt_order = self.get_order_labels(base_idx, ref_idx)
        return base_img, ref_img, one_hot_vector, order_label, gt_order, [base_age, ref_age], item
    ...
    def get_order_labels(self, base_idx, ref_idx):
        base_ranks = self.labels[base_idx]
        ref_ranks = self.labels[ref_idx]
...
def sample_nodes_within_range(labels, min_rank, max_rank):
    idxs = np.argwhere(np.logical_and(labels>=min_rank, labels<=max_rank)).flatten()
    return idxs
