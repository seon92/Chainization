...
def get_datasets_specific_edge_scenario(opt):
    loader_dict = dict()
    ...
    loader_dict['train_lb'] = DataLoader(AdiencePartialEdgeLabeled(tr_imgs, tr_ages, opt.min_age_diff, opt.info_ratio),
                                         batch_size=opt.batch_size, shuffle=True, drop_last=True, num_workers=opt.num_workers)
    loader_dict['train_ulb'] = DataLoader(AdiencePartialEdgeUnlabeled(tr_imgs, tr_ages, opt.ulb_ratio*loader_dict['train_lb'].dataset.n_pairs),
...
class AdiencePartialEdgeLabeled(Dataset):
    def __init__(self, imgs, labels, min_age_diff, info_ratio, norm_age=False, transform=None):
        super(Dataset, self).__init__()

        self.mean = [0.485, 0.456, 0.406]
        self.std = [0.229, 0.224, 0.225]
        self.normalize = transforms.Normalize(mean=self.mean, std=self.std)
        self.min_dist = min_age_diff
    ...
    def generate_training_pairs(self):
        ...
        while len(pair_set) < self.n_pairs:
            x = np.random.choice(self.n_imgs, 1)[0]
            y_cand = np.argwhere(np.abs(self.labels-self.labels[x]) > self.min_dist).flatten()
            y = np.random.choice(y_cand, 1)[0]
            if x < y:
                pair_set.add((x, y))
            elif x > y:
                pair_set.add((y, x))
...
class AdiencePartialEdgeUnlabeled(Dataset):
