...
def get_datasets_transfer_scenario(opt):
    loader_dict = dict()
    ...
    loader_dict['train_ulb'] = DataLoader(AdiencePartialNodeUnlabeled(tr_imgs, tr_ages, len(tr_ages)),
...
class AdiencePartialNodeUnlabeled(Dataset):
    ...
    def __getitem__(self, item):
        ...
        gt_order_label = self.get_gt_order(self.labels[base_idx], self.labels[ref_idx])
        ...
        return base_img, ref_img, one_hot_vector, order_label, gt_order_label, item
    ...
    def get_gt_order(self, base_age, ref_age):
        if base_age > ref_age:
            return 0
        elif base_age < ref_age:
            return 1
        elif base_age == ref_age:
            return 2
