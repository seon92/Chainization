...
class Comparator(nn.Module):
    def __init__(self, hdim, n_cls=3):
        ...
        self.fc1 = nn.Linear(2*hdim, hdim, bias=False)
...
class DRA(BaseModel):
    def __init__(self, opt):
        ...
        self.fc = nn.Linear(hdim, 1, bias=True)
        self.sigmoid = nn.Sigmoid()

    def _forward(self, base_embs, ref_embs=None):
        base_r = self.fc(base_embs)
        if ref_embs is None:
            return base_r
        ref_r = self.fc(ref_embs)
        logits = self.sigmoid(base_r - ref_r)
